; A text file is given. Read the content of the file, determine the special character with the highest frequency and display the character along with its frequency on the screen. 
...
segment data use32 class=data
    file_name db "ex_17_input.txt", 0
    access_mode db "r", 0
    format db "Character %c, frequency %d", 13, 0
    file_desc dd -1
    len equ 100
    text times (len+1) db 0
    text_len resb 1
    frequency times 16 db 0
    letter dd 0
    letter_frq dd 0
segment  code use32 class=code
    start:
        ...
        repeat:
            ...
            jmp end_iff
            special:
            sub al, ' ' ; obtain digit from ascii(al) - ascii( )
        ...
        ; go through frequency string on positions 0 to 15
        mov ecx, 16
        mov ebx, 0
        get_max:
            mov eax, 0
